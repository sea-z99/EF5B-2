...
void Hello(void)
{
	while(1)
	{
		...
		{
		case  0:break;//???????
		case  50:Led_RT_WaterOpen();delay_ms(80);delay_ms(300);Led_RT_WaterClose();break;//启动自身转向流水3*20ms,等待A 4*20ms,亮300ms,总440
		case  490:Led_RT_WaterOpen();delay_ms(80);delay_ms(300);Led_RT_WaterClose();break;//60ms等待B,启动自身转向流水4*20ms,亮300ms,总440
		...
		case  3730:Led_Tail23_BreathOpen();break;//????????λ??23 ????????255ms,???300ms??555ms
		case  4285:Led_Tail1_BreathOpen();break;//????????1 ????????255ms,???300ms??555ms
		case  4840:break;//???A??λ??6??????
...
void Bye(void)
{
	while(1)
	{
		...
		{
		case  0:break;//???????
		case  50:Led_RT_WaterOpen();delay_ms(380);Led_RT_WaterClose();break;//60ms等待B,启动自身转向流水4*20ms,间隔300ms,总440
		case  490:Led_RT_WaterOpen();delay_ms(380);Led_RT_WaterClose();break;//60ms等待B,启动自身转向流水4*20ms,间隔300ms,总440
